...
imdb.path_to_image = 'E:\Datasets\iis_datasets\VOCdevkit\VOC2012\JPEGImages'
imdb.path_to_annot = 'E:\Datasets\iis_datasets\VOCdevkit\VOC2012\SegmentationObject'
% imdb.path_to_annot = 'E:\Datasets\iis_datasets\VOCdevkit\VOC2012\Annotations'
...
for idx = idxList    
    ...
    cur_path_to_image = fullfile(imdb.path_to_image, '2007_000033.jpg');
    cur_path_to_annot = fullfile(imdb.path_to_annot, '2007_000033.png');
%     cur_path_to_annot = fullfile(imdb.path_to_annot, '2007_000033.xml');
    cur_path_to_image = strrep(cur_path_to_image,'\','/');
    cur_path_to_annot = strrep(cur_path_to_annot,'\','/');
    
    imgOrg = imread(cur_path_to_image);
    gtOrg = imread(cur_path_to_annot);
    gtOrg(gtOrg == 255) = 0;
...
%     inputs = {'data', single(im)};
    
    netbasemodel.eval(inputs) ;
    
    feaMap = gather(netbasemodel.vars(outputIdx_res7_l2norm).value);
    
    [idx3,C,sumdist3] = kmeans(feaMap,3,'Distance','cityblock','Display','final');
    print([idx3,C,sumdist3])
    ...
    prefix = sprintf('./figFolder/%s', strrep(modelName,'.mat',''));
    if ~isdir(prefix)
        mkdir(prefix);
    end
    export_fig(sprintf('%s/test.jpg', prefix));
    break;
end

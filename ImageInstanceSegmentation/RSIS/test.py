...
out_dir =os.path.join(results_dir, 'RSIS')
if not os.path.exists(out_dir):
    os.mkdir(out_dir)

excel_path = os.path.join(results_dir, 'RSIS.xlsx')
writer = pd.ExcelWriter(excel_path)

data_list = []

for name in tqdm(file_names, desc=f"RSIS"):
    ...
    outputs, _, _, mean_inference_time_for_each_object = net.predict(image_np, nums_object) # label中多少个对象，就循环预测多少次

    # label中对象的mask 与 预测的mask 一一求交叠率，再通过匈牙利匹配算法求得“完美匹配”
    
    # 构建iou矩阵
    iou_matrix = np.zeros((nums_object, nums_object)) 
    for i in range(nums_object):
        for j in range(nums_object):

...
def points_transfrom(point_list):
    new_points_list = [(-1, -1)]
    for point in point_list:
        x, y, index = point
        if index == 1:
            new_points_list.insert(0, (y, x))
        else:
            new_points_list.append((y, x))
...
def click(event, x, y, flags, param):
    ...
    if len(point_list) != num_points:
        ...
        points = points_transfrom(point_list)
        output = net.predict(image, points)

...
def get_open_file_path():
    image_path = filedialog.askopenfilename(initialdir='./images', 
...
def get_first_point_trimap(point_list, size):
    fitst_point_map = np.zeros(size)
    for point in point_list:
        x, y, index = point
        if index == 1:
            fitst_point_map[y, x] = 1
            break
    
    return fitst_point_map
...
def click(event, x, y, flags, param):
    ...
    if len(point_list) != num_points:
        cv.imshow("image", interactives_image[:, :, ::-1])
        fg, bg = points_to_trimap(point_list, interactives_image.shape[:2])
        first_point_mask = get_first_point_trimap(point_list, interactives_image.shape[:2])
        output, time = net.predict(image, fg, bg, first_point_mask)
